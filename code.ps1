...
for ($i = 5; $i -le 21; $i += 4) {
    wait_until_hour -wait_until_hour $i -czekanie_do $true
    wpisz_dane -rzad (($i-1)/4)
    wait_until_hour -wait_until_hour $i -czekanie_do $false

    Set-Location -Path "$home\Desktop\Pogoda"
